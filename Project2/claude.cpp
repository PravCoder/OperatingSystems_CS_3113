// OS Simulator - Project 2
// ==================================================
// Process Control Block Simulation Project 2 (Fixed Version)
// ==================================================
#include <iostream>
#include <vector> 
#include <string>
...
void loadJobsToMemory(queue<PCB>& newJobQueue, queue<int>& readyQueue, vector<int>& mainMemory, int maxMemory) {
    ...
    while (!newJobQueue.empty()) {
        ...
        cur_process.state = 2;  // Set to READY state

        // Store PCB metadata in main memory
        mainMemory[cur_address] = cur_process.processID;
        mainMemory[cur_address + 1] = cur_process.state;  // 2 = READY
...
void checkIOWaitingQueue(queue<int>& readyQueue, vector<int>& mainMemory, queue<IOWaitInfo>& IOWaitingQueue) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (globalClock - waitInfo.timeEntered >= waitInfo.timeNeeded) {
            ...
            cout << "print" << endl;
            cout << "Process " << mainMemory[waitInfo.startAddress] << " completed I/O and is moved to the ReadyQueue." << endl;
        } else {
...
void executeCPU(int startAddress, vector<int>& mainMemory, queue<int>& readyQueue, queue<IOWaitInfo>& IOWaitingQueue) {
    ...
    int programCounter;
    if (mainMemory[stateIndex] == 2) { // READY state
        if (mainMemory[programCounterIndex] == 0) {
    ...
    while (programCounter < dataBase && cpuCyclesThisQuantum < cpuAllocated) {
        ...
        // COMPUTE instruction
        if (opcode == 1) {
            int cycles = mainMemory[dataPointer + 1]; // cycles is second parameter
            cout << "compute" << endl;
...
int main() {
    ...
    // Print main memory after loading
    ...
    while (!readyQueue.empty() || !IOWaitingQueue.empty()) {
        if (!readyQueue.empty()) {
            ...
            // Advance clock while waiting for IO
            globalClock += contextSwitchTime;
        }
...
*/
/* 
g++ -o claude claude.cpp
./claude  < sampleInput.txt
*/